...
export default function RegisterForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      // Register the drone using only its DEVICE_ID
      ...
      if (!res.ok) throw new Error("Failed to register the drone");
      const data = await res.json();
      onRegistered(data);
    } catch {
      setError("Error registering the drone");
    } finally {
      setLoading(false);
    }
  };
        ...
        {SKINS.map((s) => (
          <button
            ...
            className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${
              skin === s.name ? "border-green-400" : "border-gray-500"
            }`}
            style={{ background: s.color }}
            onClick={() => setSkin(s.name)}
            aria-label={`Select skin ${s.name}`}
          >
            {skin === s.name && (
