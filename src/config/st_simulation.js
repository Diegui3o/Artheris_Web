...
export default function createSimulationRouter({ simState, io }) {
  const router = express.Router();
  // POST /simulate/start
  router.post("/simulate/start", (req, res) => {
    console.log(
      "[DEBUG] POST /simulate/start called. simMode:",
      simState.simMode,
      "IP:",
      req.ip,
      "Body:",
    ...
    if (simState.simInterval) {
      ...
      console.log(
        "[INFO] Cleaned previous simulation interval before starting a new."
      );
    }
    if (req.body && typeof req.body === "object") {
      Object.assign(simState.simulationParams, req.body);
      if (simState.simulationParams.mass && simState.simulationParams.g) {
        simState.simulationParams.T =
          simState.simulationParams.mass * simState.simulationParams.g;
      }
    }
    simState.simControl = {
    ...
    simState.simInterval = setInterval(() => {
      ...
      if (
        simState.simulator.phi_ref === 0 &&
        simState.simulator.theta_ref === 0
      ) {
        simState.simulator.generateTestReferences(simState.simulator.simTime);
        console.log(
          "[SIM] Using test references - phi_ref:",
          simState.simulator.phi_ref.toFixed(4),
          "theta_ref:",
          simState.simulator.theta_ref.toFixed(4)
        );
      }
      ...
      const [calculatedT, calculatedTauX, calculatedTauY, calculatedTauZ] =
        sensor.inputs || [T, 0, 0, 0];
    ...
    res.json({
      ...
      params: simState.simulationParams,
    });
  });
  ...
  function buildCombined() {
    ...
    combined.AngleRoll =
      Number(lastAngles.AngleRoll_est ?? lastAngles.AngleRoll) || 0;
    combined.AnglePitch =
      Number(lastAngles.AnglePitch_est ?? lastAngles.AnglePitch) || 0;
    combined.AngleYaw =
      Number(lastAngles.AngleYaw ?? lastAngles.AngleYaw_est) || 0;
